...
export default function Header() {
  ...
  return (
    <header className="bg-gray-900 fixed top-0 left-0 w-full z-50 shadow-lg">
      ...
      >
        {/* Logo a la izquierda, centrado un poco */}
          ...
          <PopoverGroup className="flex items-center space-x-12">
            <Popover className="relative">
              ...
              >
                Productos
                <ChevronDownIcon
                  className={`h-5 w-5 text-white transition-transform duration-200 ${popoverOpen ? 'rotate-180' : ''}`}
                  aria-hidden="true"
                />
              </PopoverButton>
        ...
        {/* Íconos a la derecha, solo visibles en pantallas grandes */}
        ...
        {/* Botón para menú móvil a la derecha */}
      ...
      {/* Menú en mobile con fondo borroso */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed top-0 right-0 w-full h-full bg-black bg-opacity-50 backdrop-blur-sm transition-all duration-500 ease-in-out">
          <div className="bg-gray-800 text-white p-4 w-64 h-full fixed right-0 top-0 transform translate-x-0 transition-transform duration-500 ease-in-out">
            {/* Cerrar el menú con una X más pequeña */}
            <button
            ...
            {/* Enlaces del menú móvil con hover */}
            <div className="flex flex-col space-y-4">
              ...
              {/* Íconos del menú móvil, centrados */}
